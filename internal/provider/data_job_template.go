...
func (d *JobTemplateDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if !data.Id.IsNull() {
		...
		url = d.client.endpoint + fmt.Sprintf("/api/v2/job_templates/%d/", id)
	}
	if !data.Name.IsNull() {
		// set url for read by name HTTP request
		name := urlParser.QueryEscape(data.Name.ValueString())
		url = d.client.endpoint + fmt.Sprintf("/api/v2/job_templates/?name=%s", name)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Client Error",
			fmt.Sprintf("Unable to read job_template, got error: %s", err))
		return
	}
